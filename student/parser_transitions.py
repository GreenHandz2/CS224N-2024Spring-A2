...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            # Shift: move the first word from buffer to the top of stack
            self.stack.append(self.buffer.pop(0))
        elif transition == "LA":
            # Left-Arc: stack top[-1] is head, second-from-top[-2] is dependent
            self.dependencies.append((self.stack[-1], self.stack[-2]))
            self.stack.pop(-2)
        elif transition == "RA":
            # Right-Arc: second-from-top[-2] is head, stack top[-1] is dependent
